...
- 其餘每行 `{"type":"turn",...}`:index / event / user / summary / source / status / at / token_count。
...
- Claude turn 來源:transcript JSONL 的真實 user prompt + `end_turn`/`stop_sequence` final response；
  tool call/result 與中間文字不進摘要，`message.usage` 依 response ID 去重後加總成 `token_count`。
- Codex turn 來源:rollout JSONL 的 `user_message` + `task_complete.last_agent_message`（fallback 為 final-phase
  `agent_message`）；tool call/result 與 commentary 不進摘要，逐筆 `last_token_usage.total_tokens` 加總成 `token_count`。

摘要 LLM 每次只收到清洗後的 user prompt 與 final response（各截 3000 字）；`token_count` 直接從上游
usage 記錄計算，不會送入摘要 LLM。
