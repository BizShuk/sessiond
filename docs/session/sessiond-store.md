...
| `token_count` | int | 該 turn 所有 model call 的 token 總和；無 usage 時省略 |
...
LLM prompt 只包含 user prompt 與 final assistant response（各截 3000 字）；commentary、tool call/result、
token usage 都不送入 LLM。只有摘要落地，assistant 原文不落地。
...
| token 用量 | 每 turn 總數 | Claude 加總去重後的 `message.usage`；Codex 加總每次 `last_token_usage.total_tokens`，不保留分類明細 |
